...
def checkReplacement(original, replacement):
    ...
    result = subprocess.run(['zsh', setupPath], stdout=subprocess.PIPE)
...
checkReplacement('device="/dev/vda"', 'device=""')
...
checkReplacement('hostname="virtual"', 'hostname=""')
checkReplacement('user="cle"', 'user=""')
